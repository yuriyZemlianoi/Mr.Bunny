...
from .exceptions import KunaException

API_VERSION = '2'
KUNA_API_URL_PREFIX = 'api/v{}'.format(API_VERSION)
KUNA_API_BASEURL = 'https://kuna.io/{}/'.format(KUNA_API_URL_PREFIX)

MARKET_PAIRS_TO_GRYVNA = ['btcuah', 'ethuah', 'xrpuah', 'ltcuah',
                          'dashuah', 'bchuah', 'xlmuah', 'gbguah',
                          'eosuah', 'tusduah', 'wavesuah']

VALID_MARKET_DATA_PAIRS = ['kunbtc', 'bchbtc', 'golgbg', 'rmcbtc', 'rbtc',
                           'evrbtc', 'foodbtc', 'hknbtc'] + \
...
class KunaAPI:
    ...
    def get_depth_market_data(self, market):
        """
        Get recent market data from server.
        :param market:
        :return:
        ...
        return self.request('depth', args=args)
    ...
    def request(self, path, args=None, method='GET', is_user_method=False):
        ...
        try:
            ...
            raise KunaException(response)

        result = response.json()

        if result and isinstance(result, dict) and result.get('error'):
            raise KunaException(result)
        elif response.status_code not in [200, 201, 202]:
            raise KunaException(response.reason)
        return result
